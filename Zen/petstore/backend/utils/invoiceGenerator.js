...
export const generateInvoice = async (req, res) => {
  try {
    ...
    if (queryToken) {
      try {
        req.user = jwt.verify(queryToken, process.env.JWT_SECRET);
      } catch {
    ...
    doc.moveTo(M,    secY + 16).lineTo(M + 200,    secY + 16)
       .strokeColor('#F97316').lineWidth(1.5).stroke();
    doc.moveTo(col2, secY + 16).lineTo(col2 + 200, secY + 16)
       .strokeColor('#F97316').lineWidth(1.5).stroke();
    ...
    doc.rect(M, rowY, 220, 24).fill(badgeBg);
