...
const generateOrderNumber = () => 'PET-' + Date.now() + '-' + Math.floor(Math.random() * 1000);
...
router.post('/', authenticate, async (req, res) => {
  ...
  try {
    ...
    const {
      shipping, payment_method = 'razorpay', coupon_code,
      notes, payment_id = null, razorpay_order_id = null,
    } = req.body;
    ...
    const [userRows] = await conn.query('SELECT email, name FROM users WHERE id = ?', [req.user.id]);
    ...
    let subtotal = cartItems.reduce((s, i) => s + (i.discount_price || i.price) * i.quantity, 0);
    let discount = 0;
    ...
    const isTN = ['tamil nadu', 'tamilnadu', 'tn'].includes((shipping.state || '').toLowerCase().trim());
    ...
    const [orderResult] = await conn.query(
      ...
      [
        ...
        payment_id ? 'paid' : 'pending', payment_id || null,
        shipping.name, shipping.email, shipping.phone,
        shipping.address, shipping.city, shipping.state, shipping.pincode, notes || null,
      ]
    );
    ...
    sendOrderConfirmationEmail({
      order_id: orderResult.insertId, order_number,
      customer_name: customerName, customer_email: customerEmail,
      items: cartItems.map(i => ({ name: i.name, quantity: i.quantity, price: i.discount_price || i.price })),
      subtotal, discount, shipping_amount, tax, total, shipping, payment_method,
