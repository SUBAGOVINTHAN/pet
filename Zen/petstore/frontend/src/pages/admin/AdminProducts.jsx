...
export default function AdminProducts() {
  ...
  const openEdit = (p) => {
    ...
    setImageFiles([]);
    // Show existing extra images as previews
    const extras = Array.isArray(p.images) ? p.images : [];
    setImagePreviews(extras.map(getImageUrl));
    setModal(true);
  };
  ...
  const handleImageChange = (e) => {
    ...
    setImageFiles(files);
    // First file = main image preview
    setImageFile(files[0]);
    setImagePreview(URL.createObjectURL(files[0]));
    // Rest = extra previews
    setImagePreviews(files.slice(1).map(f => URL.createObjectURL(f)));
  };
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 600, maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 8, color: '#555' }}>
                Product Images (First = Main, rest = gallery)
              </label>

              {/* Main image preview + extra previews */}
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 10 }}>
                <div style={{ width: 100, height: 100, borderRadius: 10, border: '2px dashed #F97316', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#FFF7F0' }}>
                  {imagePreview ? (
                    <img src={imagePreview} alt="main" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={() => setImagePreview(null)} />
                  ) : (
                    <div style={{ textAlign: 'center', color: '#F97316', fontSize: 11 }}>
                      <ImageOff size={20} style={{ margin: '0 auto 4px', display: 'block' }} />
                      Main
                    </div>
                  )}
                </div>
                ...
                {imagePreviews.map((src, i) => (
                  <div key={i} style={{ width: 80, height: 80, borderRadius: 8, border: '1.5px solid #eee', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9f9f9' }}>
              ...
              <p style={{ fontSize: 12, color: '#9CA3AF' }}>
                Select up to 5 images. First image = main photo, rest = gallery thumbnails.<br />
                {editing ? 'Leave empty to keep current images.' : ''}
              </p>
            </div>
